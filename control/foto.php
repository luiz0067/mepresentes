...
<?php
	...
	if ($_POST) {
		...
		if ($_POST['acao']=='Inserir'){
			$sql = "insert into foto (imagem,titulo,id_categoria,obs) values ('".$imagem."','".$_POST["titulo"]."','".$_POST["id_categoria"]."','".$_POST["obs"]."');";
			$reultado = mysql_query($sql, $link);
			echo $sql;
			if($reultado){
			$erro = "Dados salvos com sucesso!";
				header("Location: ../adm/principal.php?pg=cadastro_foto");
			} 
			else
			{
		...
		$sql = "update imagem set foto='".$imagem."',titulo='".$_POST["titulo"]."',obs='".$_POST["obs"]."' where (id=".$_POST["id"].");";
		echo $sql;
		$reultado = mysql_query($sql, $link);
		if($sql){
				$erro = "Dados salvos com sucesso!";
