...
<?php
	...
	if ($_POST) {
		...
		if ($_POST['acao']=='Inserir'){
			$sql = "insert into marcas (imagem,titulo,link_imagem) values ('".$imagem."','".$_POST["titulo"]."','".$_POST["link_imagem"]."');";
			$reultado = mysql_query($sql, $link);
			echo $sql;
			if($reultado){
			$erro = "Dados salvos com sucesso!";
				header("Location: ../adm/principal.php?pg=cadastro_marcas");
			} 
			else
			{
		...
		$sql = "update imagem set marcas='".$imagem."',titulo='".$_POST["titulo"]."',link_imagem='".$_POST["link_imagem"]."' where (id=".$_POST["id"].");";
		echo $sql;
		$reultado = mysql_query($sql, $link);
		if($sql){
				$erro = "Dados salvos com sucesso!";
